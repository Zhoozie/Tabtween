...
import FontSelect from '@/newtab/components/settings/FontSelect.vue'
  ...
  <PanelShell v-model:open="open" title="设置" width="min(760px, 95vw)" height="min(80vh, 86vh)">
    ...
    <div class="flex min-h-0 flex-1">
      ...
      <div class="flex min-h-0 min-w-0 flex-1 flex-col">
        <!-- 设置项搜索 -->
        <div class="shrink-0 border-b px-4 py-2" :style="{ borderColor: 'var(--color-border)' }">
          <input
            v-model="searchQuery"
            type="text"
            placeholder="搜索设置项…"
          ...
          >
            ...
            <div v-if="showSearch">
              ...
              >
                ...
                <div class="text-xs opacity-60">{{ r.description }} · {{ r.categoryLabel }}</div>
              </button>
            </div>
              ...
              <SettingGroup title="主题">
                ...
                <div class="mt-3">
                  ...
                  <FontSelect
                    ...
                    preview-text="Aa"
                    @update:model-value="settingsStore.updateAppearance({ fontFamily: $event })"
                  />
                </div>
                <div class="mt-3">
            ...
            <div v-else-if="activeCategory === 'display'">
              ...
              <SettingGroup title="时钟">
                <!-- 顶部：显示时钟开关 -->
                <SettingToggle
                  v-for="t in CLOCK_VISIBLE_TOGGLE"
                  :key="t.key"
                ...
                <!-- 下方：样式选择 + 样式专属设置（关闭显示时全部置灰） -->
                <div :class="settings.clock.visible ? '' : 'pointer-events-none opacity-40'">
                  <!-- 时钟样式选择 -->
                  <div class="mt-3">
                  ...
                  <!-- 数字时钟专属设置 -->
                  <template v-if="settings.clock.style === 'digital'">
                    <div class="mt-3">
                      <p class="mb-1 text-xs opacity-70">显示内容</p>
                      <div class="grid grid-cols-3 gap-2">
                        <button
                          v-for="t in DIGITAL_TOGGLES"
                          :key="t.key"
                          type="button"
                          class="rounded-full border px-3 py-1.5 text-sm transition-colors"
                          :style="
                            settings.clock[t.key]
                              ? {
                    ...
                    <div class="mt-3">
                      ...
                      <FontSelect
                        ...
                        preview-text="12:34"
                        @update:model-value="settingsStore.updateClock({ clockFont: $event })"
                      />
                    </div>
                  </template>

                  <!-- 模拟时钟专属设置 -->
                  <template v-else-if="settings.clock.style === 'analog'">
                    <div class="mt-3">
                      <SettingToggle
                        :model-value="settings.clock.showTicks"
                        label="刻度显示"
                        description="在表盘周边绘制刻度"
                        @update:model-value="
                          settingsStore.updateClock({
                            showTicks: $event
                          })
                    ...
                    <div class="mt-2">
                      <p class="mb-1 text-xs opacity-70">刻度样式</p>
                      <SettingRadio
                        :model-value="settings.clock.tickStyle"
                        :options="TICK_STYLE_OPTIONS"
                        :disabled="!settings.clock.showTicks"
                        @update:model-value="
                          settingsStore.updateClock({ tickStyle: $event as TickStyle })
                        "
                      />
                    </div>
                    <div class="mt-3">
                      <SettingToggle
                        :model-value="settings.clock.showSecondsHand"
                        label="秒针显示"
                        description="显示红色秒针"
                        @update:model-value="
                          settingsStore.updateClock({
                            showSecondsHand: $event
                          })
                        "
                      />
                    </div>
                    <div class="mt-2">
                      <p class="mb-1 text-xs opacity-70">秒针颜色</p>
                      <SettingColorPicker
                        :model-value="settings.clock.secondHandColor"
                        :presets="SECOND_HAND_COLOR_PRESETS"
                        :disabled="!settings.clock.showSecondsHand"
                        @update:model-value="
                          settingsStore.updateClock({ secondHandColor: String($event) })
                        "
                      />
                    </div>
                    <div class="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 md:grid-cols-3">
                      <SettingToggle
                        v-for="t in ANALOG_TOGGLES"
                        :key="t.key"
                        :model-value="Boolean(settings.clock[t.key])"
                        :label="t.label"
                        @update:model-value="
              ...
              <SettingGroup title="快捷访问">
                <SettingToggle
                  ...
                  @update:model-value="settingsStore.updateDisplay({ showQuickAccess: $event })"
                />
              </SettingGroup>
            </div>
